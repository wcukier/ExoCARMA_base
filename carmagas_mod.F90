...
contains
  ...
  subroutine CARMAGAS_Create(carma, igas, name, wtmol_dif, icomposition,  &
    rc,&
       shortname, &
    dgc_threshold, ds_threshold)
    type(carma_type), intent(inout)       :: carma           !! the carma object
    integer, intent(in)                   :: igas            !! the gas index
    character(*), intent(in)              :: name            !! the gas name, maximum of 255 characters
    real(kind=f), intent(in)              :: wtmol_dif           !! the gas molecular weight [g/mol]
    ...
    carma%f_gas(igas)%f_wtmol_dif    = wtmol_dif
  ...
  !! @see CARMAGAS_Create
  !! @see CARMA_GetGas
  subroutine CARMAGAS_Get(carma, igas, rc, name, shortname, wtmol_dif, icomposition, dgc_threshold, ds_threshold)
    ...
    real(kind=f), optional, intent(out)         :: wtmol_dif         !! the gas molecular weight [g/mol]
    ...
    if (present(wtmol_dif))    wtmol_dif        = carma%f_gas(igas)%f_wtmol_dif
  ...
  subroutine CARMAGAS_Print(carma, igas, rc)
    ...
    if (carma%f_do_print) then
      call CARMAGAS_Get(carma, igas, rc, name=name, shortname=shortname,  &
